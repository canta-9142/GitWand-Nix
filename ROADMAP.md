...
- **dev-server / Rust read-file parity (#185)** — `read_file` is `std::fs::read_to_string` in Rust, which rejects a non-UTF-8 file, while the dev-server's `readFileSync(path, "utf-8")` never throws and substitutes `U+FFFD`. A bug that made every conflict in a repository unresolvable in the packaged app was therefore invisible under `pnpm dev:web` (fixed in v3.10.1, but the blind spot remains). Make the dev-server fail the same way and add `tests/parity/read-file.test.mjs` covering agreement on failure, not just on success; check `read_file_at_revision` for the same drift
